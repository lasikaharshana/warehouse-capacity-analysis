...
The same analysis as a working Excel tool — change an input and every result recalculates.

![Excel dashboard](excel/screenshots/01_dashboard.png)

| Sheet | What it does |
|---|---|
| **Dashboard** | KPI cards and charts filtered by storage condition, principal and velocity class |
| **SKU Lookup** | Pick any SKU to see its pallet build, demand, classification, stock and expiry alerts |
| **Rack Scenario** | Change the bay mix and test whether the stock fits, what it costs, and the payback |
| **Cycle Count Planner** | Set count frequencies by value class and see the weekly workload |

The Rack Scenario sheet shows the key idea directly: a layout with *more* pallet positions can still fail, because capacity depends on having positions of the right height.

![Rack scenario — feasible](excel/screenshots/03_rack_scenario.png)

**Excel techniques used:** Power Query (import, merge, data type control), structured table references, XLOOKUP and two-way INDEX/MATCH, SUMIFS and COUNTIFS with wildcard filters, dynamic arrays (UNIQUE, SORT, SORTBY, HSTACK, VSTACK), SUMPRODUCT, data validation, conditional formatting and sheet protection.

**[Download the workbook](excel/warehouse_capacity_dashboard.xlsx)** — requires Microsoft 365 or Excel 2021+. The data is saved inside the file, so it works without refreshing; the Power Query connections point to the author's local files.
...
Python · pandas · NumPy · Matplotlib · Jupyter · openpyxl · Excel (Power Query, dynamic arrays)
...
├── excel/         interactive Excel model and screenshots
